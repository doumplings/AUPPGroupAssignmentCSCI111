...
ASSETS_PATH = OUTPUT_PATH / Path(r"D:\Coding\Python\StuManSys\build\assets\frame5")


def relative_to_assets(path: str) -> Path:
    return ASSETS_PATH / Path(path)

def DeleteStudentPage(studentId):
    from backend.getStudent import getStudent
    from backend.delStudent import deleteStudent
    ...
    def delete_clicked():
        ...
        if isDeleted == True:
            window.destroy()
            successPopUp('Student Deleted', SearchStudentPage, 'DELETE')
        else: 
            window.destroy()
            failPopup('Id not found', DeleteStudentPage)
