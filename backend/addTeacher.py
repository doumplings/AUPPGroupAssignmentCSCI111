...
def addTeacher(id, first_name, last_name, email, phonenum, subject):
    conn = None
    try:
    ...
    if conn is not None:
        # STEP 4 Create Query
        query = "INSERT INTO teachers(id, first_name, last_name, email, phonenum, subject) VALUES(%s,%s,%s,%s,%s,%s)"

        try:
            # STEP - 5 TAKE INPUT FROM USER
            teacher = [id, first_name, last_name, email, phonenum, subject]
            ...
            try:
                ...
                noofrecoredinsert = cursor.execute(query, teacher)
